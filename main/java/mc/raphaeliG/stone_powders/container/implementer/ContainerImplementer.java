...
public class ContainerImplementer extends Container {
	
	private TileEntityImplementer tileEntity;
	private IItemHandler handler;
	
	public ContainerImplementer(IInventory playerInventory, TileEntityImplementer tileEntity) {
		this.tileEntity = tileEntity;
		handler = this.tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
		addSlotToContainer(new SlotInputFuel(handler, TileEntityImplementer.SLOT_INPUT_FUEL, 20, 35));
		addSlotToContainer(new SlotInputStone(handler, TileEntityImplementer.SLOT_INPUT_STONE, 56, 49));
		addSlotToContainer(new SlotInputPowder(handler, TileEntityImplementer.SLOT_INPUT_POWDER, 56, 20));
		addSlotToContainer(new SlotOutput(handler, TileEntityImplementer.SLOT_OUTPUT_STONE, 116, 35));
